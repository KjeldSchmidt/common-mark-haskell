...
tagChars = map (\x -> (x, (tag . ord) x))
    where
        tag x
            ...
            | x == 13 = CarriageReturn
            | x == 32 = Space
            | x `elem` [11, 12] = WhiteSpace
            | x `elem` ugc "Zs" = UnicodeWhiteSpace
